...
def saltstack_all(req):
    ...
    if req.method == "POST":
           ...
           # node_list=node.split(',')
            for node in host:
                ...
                if cmd in allow_cmd:
                    ...
                    try:
                        ret = salt.client.LocalClient().cmd(
                            node, 'cmd.run', [cmd])
                        print "=================="
                        if len(ret) < 1:
                            err = str(node) + " " * 24 + \
                                'minions is not running'
                            ret_err.append(err)
                            print ret_err
        ...
        if True:
            ...
            salt_key = proc.stdout.read()
            try:
                node=salt_key.split("Denied Keys")[0].split("\n").pop(1)
            except:
                node = 'node1'
            cmd = 'grains.items'
            ...
            '''
            if 'defaultencoding' in grains_info or 'defaultlanguage' in grains_info:
...
def salt_key(req):
    ...
    # return
    # render_to_response('saltstack.html',{"salt_key":salt_key},context_instance=RequestContext(req))
...
def ssh_remote_manager(req):
    ...
    try:
        ...
        subprocess.call('yum install -y python-paramiko',
                        shell=True) | subprocess.call('easy_install paramiko', shell=True)
    import sys
    import os
    if req.method == "GET":
